...
static wmg_bool_t wifimanager_init_flag = WMG_FALSE;
...
int __wifi_on(void **call_argv,void **cb_argv)
{
	...
	ret = wifimg_object->switch_mode(*mode, WMG_TRUE);
...
int __wifi_off(void **call_argv,void **cb_argv)
{
	...
	if(wifimg_object->is_init()){
		ret = wifimg_object->switch_mode(*mode, WMG_FALSE);
		if ((ret == WMG_STATUS_UNHANDLED) || (ret == WMG_STATUS_SUCCESS)) {
			WMG_DEBUG("switch wifi mode success\n");
			WMG_INFO("wifi mode %s off success\n",wmg_wifi_mode_to_str(*mode));
			if(wifimg_object->current_mode_bitmap == 0) {
				WMG_INFO("all wifi mode is off, deinit now\n");
				wifimg_object->deinit();
			}
			ret = WMG_STATUS_SUCCESS;
		} else {
			WMG_ERROR("failed to switch wifi mode\n");
			WMG_ERROR("wifi mode %s off failed\n",wmg_wifi_mode_to_str(*mode));
			ret = WMG_STATUS_FAIL;
		}
	} else {
...
{
	wifimg_object_t* wifimg_object = get_wifimg_object();
	wifi_monitor_config_t *monitor_config = (wifi_monitor_config_t *)call_argv[0];

	wmg_status_t *cb_status = (wmg_status_t *)cb_argv[0];

	CHECK_MODE_STATUS(MONITOR_BITMAP);

	*cb_status = wifimg_object->monitor_enable(monitor_config);
	if(*cb_status != WMG_STATUS_SUCCESS) {
...
{
	wifimg_object_t* wifimg_object = get_wifimg_object();
	int *find_second = (int *)call_argv[0];
	char *types = (char *)call_argv[1];
	wmg_status_t *cb_status = (wmg_status_t *)cb_argv[0];

	CHECK_MODE_STATUS(P2P_BITMAP);

	*cb_status = wifimg_object->p2p_find(*find_second, types);
	if(*cb_status != WMG_STATUS_SUCCESS){
		WMG_ERROR("wifi p2p find fail\n");
	}
...
int __wifi_p2p_get_peers(void **call_argv,void **cb_argv)
{
	...
	uint32_t *p2p_num = (uint32_t *)call_argv[1];
	uint32_t *p2p_arr_size = (uint32_t *)call_argv[2];
	wmg_status_t *cb_status = (wmg_status_t *)cb_argv[0];

	CHECK_MODE_STATUS(P2P_BITMAP);
	*cb_status = wifimg_object->p2p_get_peers(p2p_peers, p2p_num, *p2p_arr_size);
	if(*cb_status != WMG_STATUS_SUCCESS){
		WMG_ERROR("wifi p2p get peers fail\n");
	}
...
int __wifi_set_mac(void **call_argv,void **cb_argv)
{
	...
	*cb_status = wifimg_object->send_expand_cmd(expand_cmd, NULL, NULL);
	if(*cb_status != WMG_STATUS_SUCCESS) {
...
int __wifi_get_mac(void **call_argv,void **cb_argv)
{
	...
	*cb_status = wifimg_object->send_expand_cmd(expand_cmd, NULL, mac_addr);
	if(*cb_status != WMG_STATUS_SUCCESS) {
...
{
	wifimg_object_t* wifimg_object = get_wifimg_object();
	char *country_code = (char *)call_argv[0];
	wmg_status_t *cb_status = (wmg_status_t *)cb_argv[0];
	char expand_cmd[128] = {0};

	if (strlen(country_code) > 2) {
		WMG_ERROR("wifi set country code: %s invalid\n", country_code);
		*cb_status = WMG_STATUS_INVALID;
		return *cb_status;
	}

#ifdef OS_NET_LINUX_OS
	strcat(expand_cmd, "linux:sccode:");
#endif
#ifdef OS_NET_XRLINK_OS
	strcat(expand_cmd, "xrlink:sccode:");
#endif
#ifdef OS_NET_FREERTOS_OS
	strcat(expand_cmd, "freertos:sccode:");
#endif

	strcat(expand_cmd, country_code);
...
#ifdef OS_NET_LINUX_OS
	strcat(expand_cmd, "linux:gccode:");
#endif
#ifdef OS_NET_XRLINK_OS
	strcat(expand_cmd, "xrlink:gccode:");
#endif
#ifdef OS_NET_FREERTOS_OS
	strcat(expand_cmd, "freertos:gccode:");
#endif

	*cb_status = wifimg_object->send_expand_cmd(expand_cmd, NULL, country_code);
	if(*cb_status != WMG_STATUS_SUCCESS) {
		WMG_ERROR("wifi get country code fail\n");
	}
...
int __wifi_linkd_protocol(void **call_argv,void **cb_argv)
{
	...
	if(*mode == WMG_LINKD_MODE_SOFTAP) {
		...
		ret = wifimg_object->switch_mode(WIFI_AP, WMG_TRUE);
		if ((ret == WMG_STATUS_UNHANDLED) || (ret == WMG_STATUS_SUCCESS)) {
...
#ifdef SUPPORT_AP_MODE
	if(*mode == WMG_LINKD_MODE_SOFTAP) {
		if (wifimg_object->ap_disable() == WMG_STATUS_SUCCESS) {
...
int __wifi_send_expand_cmd(void **call_argv,void **cb_argv)
{
	...
	void *expand_params = (void *)call_argv[1];
	void *expand_cb = (void *)call_argv[2];
	wmg_status_t *cb_status = (wmg_status_t *)cb_argv[0];

	if(expand_cmd) {
		*cb_status = wifimg_object->send_expand_cmd(expand_cmd, expand_params, expand_cb);
		if(*cb_status != WMG_STATUS_SUCCESS) {
			WMG_ERROR("wifi send expand cmd fail\n");
		}
	} else {
...
#else
	[WMG_PLA_ACT_WIFI_SET_MAC] = {__wifi_unspport_mode_function, "__wifi_unspport_mode_function"},
	[WMG_PLA_ACT_WIFI_GET_MAC] = {__wifi_unspport_mode_function, "__wifi_unspport_mode_function"},
	[WMG_PLA_ACT_WIFI_SET_COUNTRY_CODE] = {__wifi_unspport_mode_function, "__wifi_unspport_mode_function"},
	[WMG_PLA_ACT_WIFI_GET_COUNTRY_CODE] = {__wifi_unspport_mode_function, "__wifi_unspport_mode_function"},
...
act_func_t p2p_action_table[] = {
	...
	[WMG_P2P_ACT_GET_PEERS] = {__wifi_p2p_get_peers, "__wifi_p2p_get_peers"},
...
act_func_t p2p_action_table[] = {
	...
	[WMG_P2P_ACT_GET_PEERS] = {__wifi_unspport_mode_function, "__wifi_unspport_mode_function"},
...
wmg_status_t __wifimanager_init(void)
{
	if(!wifimanager_init_flag) {
		...
		wifimanager_init_flag = WMG_TRUE;
	}
...
wmg_status_t __wifimanager_deinit(void)
{
	...
	wifimanager_init_flag = WMG_FALSE;
	return WMG_STATUS_SUCCESS;
}
 ...
 */
static uint8_t support_list[MAX_SUPPORT_MODE_NUM] = {
	0,                                         //00 mode(0000): none mode
#ifdef SUPPORT_STA_MODE
	WIFI_STATION,                              //01 mode(0001): sta mode (enable)
#else
	0,                                         //01 mode(0001): sta mode (disable)
#endif
#ifdef SUPPORT_AP_MODE
	WIFI_AP,                                   //02 mode(0010): ap mode (enable)
#else
	0,                                         //02 mode(0010): ap mode (disable)
#endif
#ifdef SUPPORT_STA_AP_MODE
	WIFI_STATION_AP,                           //03 mode(0011): sta + ap mode (enable)
#else
	0,                                         //03 mode(0011): sta + ap mode (disable)
#endif
#ifdef SUPPORT_MONITOR_MODE
	WIFI_MONITOR,                              //04 mode(0100): monitor mode (enable)
#else
	0,                                         //04 mode(0100): monitor mode (enable)
#endif
	0,                                         //05 mode(0101): sta + monitor (unsupport)
	0,                                         //06 mode(0110): ap + monitor (unsupport)
	0,                                         //07 mode(0111): sta + ap + monitor (unsupport)
#ifdef SUPPORT_P2P_MODE
	WIFI_P2P,                                  //08 mode(1000): p2p mode (enable)
#else
	0,                                         //08 mode(1000): p2p mode (disable)
#endif
#ifdef SUPPORT_STA_P2P_MODE
	WIFI_STATION_P2P,                          //09 mode(1001): sta + p2p mode (enable)
#else
	0,                                         //09 mode(1001): sta + p2p mode (disable)
#endif
	0,                                         //10 mode(1010): ap + p2p mode (unsupport)
	0,                                         //11 mode(1011): sta + ap + p2p mode (unsupport)
	0,                                         //12 mode(1100): monitor + p2p mode (unsupport)
	0,                                         //13 mode(1101): sta + monitor + p2p mode (unsupport)
	0,                                         //14 mode(1110): ap + monitor + p2p mode (unsupport)
	0                                          //15 mode(1111): sta + ap + monitor + p2p mode (unsupport)
};

/* In principle, every os layer needs to implement this function */
/* Only a weak os function is provided here to prevent compilation errors */
uint8_t* __attribute__((weak)) module_mode_support_list_register(void)
{
	return support_list;
}
 ...
 * 1.Return module name
 * 2.Is there any supporting information matching this module
 *   (1) no match: module_flag 0 (D): use the default
 *   (2) match   : module_flag 2 (M): use matching
 * */
wmg_status_t __attribute__((weak)) module_name_register(char* module_name)
{
	sprintf(module_name, "%s %s", "UNKNOWN", "(D)");
	return WMG_STATUS_FAIL;
}

static wmg_bool_t compared_mode_support_list(uint8_t mode_table)
{
	uint8_t *table_p = wifimg_object.wmg_mode_support_list;

	if(mode_table == table_p[mode_table]){
		return WMG_TRUE;
	} else {
		return WMG_FALSE;
	}
...
/*
 * return
 * WMG_STATUS_SUCCESS: member are legitimate and supportive
 * WMG_STATUS_FAIL: member is illegal
 * WMG_STATUS_UNSUPPORTED: do not support
 * */
static wmg_status_t chech_support_list_member(uint8_t *support_list, wifi_mode_t wifi_mode)
{
	if(support_list[wifi_mode] == wifi_mode) {
		return WMG_STATUS_SUCCESS;
	} else if (support_list[wifi_mode] == 0) {
		return WMG_STATUS_UNSUPPORTED;
	} else {
		return WMG_STATUS_FAIL;
	}
...
static wmg_status_t support_list_to_char(uint8_t *support_list, char *support_list_char)
{
	wmg_status_t ret = WMG_STATUS_FAIL;

	strcat(support_list_char, "(");
	ret = chech_support_list_member(support_list, WIFI_STATION);
	if(ret == WMG_STATUS_SUCCESS) {
		strcat(support_list_char, "sta | ");
	} else if (ret == WMG_STATUS_UNSUPPORTED) {
		strcat(support_list_char, "--- | ");
	} else {
		return ret;
	}
	ret = chech_support_list_member(support_list, WIFI_AP);
	if(ret == WMG_STATUS_SUCCESS) {
		strcat(support_list_char, "ap | ");
	} else if (ret == WMG_STATUS_UNSUPPORTED) {
		strcat(support_list_char, "-- | ");
	} else {
		return ret;
	}
	ret = chech_support_list_member(support_list, WIFI_MONITOR);
	if(ret == WMG_STATUS_SUCCESS) {
		strcat(support_list_char, "mon | ");
	} else if (ret == WMG_STATUS_UNSUPPORTED) {
		strcat(support_list_char, "--- | ");
	} else {
		return ret;
	}
	ret = chech_support_list_member(support_list, WIFI_P2P);
	if(ret == WMG_STATUS_SUCCESS) {
		strcat(support_list_char, "p2p | ");
	} else if (ret == WMG_STATUS_UNSUPPORTED) {
		strcat(support_list_char, "--- | ");
	} else {
		return ret;
	}
	ret = chech_support_list_member(support_list, WIFI_STATION_AP);
	if(ret == WMG_STATUS_SUCCESS) {
		strcat(support_list_char, "sta-ap | ");
	} else if (ret == WMG_STATUS_UNSUPPORTED) {
		strcat(support_list_char, "------ | ");
	} else {
		return ret;
	}
	ret = chech_support_list_member(support_list, WIFI_STATION_P2P);
	if(ret == WMG_STATUS_SUCCESS) {
		strcat(support_list_char, "sta-p2p");
	} else if (ret == WMG_STATUS_UNSUPPORTED) {
		strcat(support_list_char, "-------");
	} else {
		return ret;
...
static wmg_status_t update_mode_support_list(uint8_t *update_support_list, uint8_t *support_list, char *result)
{
	int i = 0;
	uint8_t actual_list[MAX_SUPPORT_MODE_NUM] = {0};

	memcpy(actual_list, update_support_list, sizeof(actual_list));
	for(; i < MAX_SUPPORT_MODE_NUM; i++) {
		if(update_support_list[i] != support_list[i]) {
			actual_list[i] = 0;
		}
	}
	return support_list_to_char(actual_list, result);
}
...
static void wmg_mode_object_register(int mode_num, uint8_t mode_bitmap, mode_object_t *mode_object)
{
	...
	if(wifimg_object.mode_object[mode_num] != NULL) {
		if(wifimg_object.mode_object[mode_num]->init != NULL) {
			if(wifimg_object.mode_object[mode_num]->init(wifimg_object.wmg_msg_cb, wifimg_object.wmg_msg_cb_arg)){
...
{
	wmg_status_t ret = WMG_STATUS_FAIL;
	char support_list_char_buff[256] = {0};

	if(wifimg_object.init_flag == WMG_FALSE) {
		os_net_mutex_create(&wifimg_object.mutex_lock);

/* wifi driver name and support mode */
		module_name_register(wifimg_object.module_name);
		wifimg_object.module_mode_support_list = module_mode_support_list_register();
/* wifimanager support mode */
		...
		WMG_INFO("* %s\n", WMG_DECLARE);
		WMG_INFO("* version: %s\n", WMG_VERSION);
		WMG_INFO("* module name: %s\n", wifimg_object.module_name);
		if(!support_list_to_char(wifimg_object.wmg_mode_support_list, support_list_char_buff)) {
			WMG_INFO("* wifimg support mode:%s\n", support_list_char_buff);
		} else {
			WMG_ERROR("failed to parse support_list_char_buff\n");
		}
		memset(support_list_char_buff, 0, 256);
		if(!support_list_to_char(wifimg_object.module_mode_support_list, support_list_char_buff)) {
			WMG_INFO("* module support mode:%s\n", support_list_char_buff);
		} else {
			WMG_ERROR("failed to parse support_list_char_buff\n");
		}
/* actually support mode */
		memset(support_list_char_buff, 0, 256);
		if(!update_mode_support_list(wifimg_object.wmg_mode_support_list, wifimg_object.module_mode_support_list, support_list_char_buff)) {
			WMG_INFO("* actual support mode:%s\n", support_list_char_buff);
		} else {
			WMG_ERROR("failed to parse support_list_char_buff\n");
		}
		WMG_INFO("**************************************\n");
...
static void wifimg_deinit(void)
{
	if(wifimg_object.init_flag == WMG_TRUE) {
		...
		memset(wifimg_object.module_name, 0, MODULE_NAME_MAX_LEN);
		sprintf(wifimg_object.module_name, "%s", "UNKNOWN");
		wifimg_object.module_mode_support_list = NULL;
...
/*
 * All mode switching needs to go through this function
 * whether it is turning on a certain mode or turning off a certain mode.
 */
static wmg_status_t wifimg_switch_mode(wifi_mode_t switch_mode, wmg_bool_t on_flag)
{
	uint8_t enable_mode_bitmap = 0, disable_mode_bitmap = 0, after_mode_bitmap = 0;
	...
	/* Determine whether the switch mode is supported */
	if(((wifimg_object.support_mode_bitmap & (uint8_t)switch_mode) == 0) && (on_flag != WMG_FALSE) && (switch_mode != WIFI_MODE_UNKNOWN)) {
		WMG_ERROR("switch mode %s unsupport\n", wmg_wifi_mode_to_str((uint8_t)switch_mode));
		return WMG_STATUS_UNSUPPORTED;
	}

	/* check whether the current mode needs to be switch */
	if(on_flag) {
		if((uint8_t)switch_mode & wifimg_object.current_mode_bitmap) {
			WMG_DEBUG("Current mode 0x%x, no need to enable mode 0x%x\n", wifimg_object.current_mode_bitmap, (uint8_t)switch_mode);
			return WMG_STATUS_UNHANDLED;
		}
	} else {
		if((uint8_t)switch_mode & ~(wifimg_object.current_mode_bitmap)) {
			WMG_DEBUG("Current mode 0x%x, no need to disable mode 0x%x\n", wifimg_object.current_mode_bitmap, (uint8_t)switch_mode);
			return WMG_STATUS_UNHANDLED;
		}
	}

	/* Determine which modes need to be turned on/off */
	if(on_flag) {
		enable_mode_bitmap = (uint8_t)switch_mode;
		/*
		 * The mode logic that needs to be turned off for coexistence and non-coexistence is different
		 * Here is how to determine whether coexistence is supported
		 */
		if(compared_mode_support_list(((uint8_t)switch_mode | wifimg_object.current_mode_bitmap))) {
			disable_mode_bitmap = ((~((uint8_t)switch_mode | wifimg_object.current_mode_bitmap)) & wifimg_object.current_mode_bitmap);
		} else {
			disable_mode_bitmap = ((~((uint8_t)switch_mode)) & wifimg_object.current_mode_bitmap);
		}
	} else {
		enable_mode_bitmap = 0;
		/* off all mode */
		if(switch_mode == WIFI_MODE_UNKNOWN) {
			disable_mode_bitmap = wifimg_object.current_mode_bitmap;
		} else {
			disable_mode_bitmap = (uint8_t)switch_mode;
		}
	}
	after_mode_bitmap = ((wifimg_object.current_mode_bitmap & (~disable_mode_bitmap)) | enable_mode_bitmap);

	WMG_DEBUG("switch(%s) mode: bitmap:0X:%x\n", on_flag ? " on" : "off" ,(uint8_t)switch_mode);
	WMG_DEBUG("current     mode: bitmap:0X:%x\n", wifimg_object.current_mode_bitmap);
	WMG_DEBUG("enable      mode: bitmap:0X:%x\n", enable_mode_bitmap);
	WMG_DEBUG("disable     mode: bitmap:0X:%x\n", disable_mode_bitmap);
	WMG_DEBUG("after       mode: bitmap:0X:%x\n", after_mode_bitmap);

	os_net_mutex_lock(&wifimg_object.mutex_lock);

	/* which modes need to be turned off */
	...
	/* which modes need to be turned on */
	...
	if (enable_mode_bitmap & P2P_BITMAP) {
	...
	if(after_mode_bitmap == wifimg_object.current_mode_bitmap){
...
#ifdef SUPPORT_MONITOR_MODE
static wmg_status_t wifimg_monitor_enable(wifi_monitor_config_t *monitor_config)
{
	return wifimg_mode_ctl_cmd(MONITOR_MODE_NUM , WMG_MONITOR_CMD_ENABLE, (void *)monitor_config, NULL);
}
...
static wmg_status_t wifimg_p2p_find(int find_second, char *types)
{
	find_para_t cmd_para = {
...
static wmg_status_t wifimg_p2p_get_peers(wifi_p2p_peers_t *p2p_peers, uint32_t *p2p_num, uint32_t p2p_arr_size)
{
	wmg_status_t ret, cb_msg;
	find_results_para_t cmd_para = {
		.peers = p2p_peers,
		.num = p2p_num,
		.arr_size = p2p_arr_size,
	};

	return wifimg_mode_ctl_cmd(P2P_MODE_NUM , WMG_P2P_CMD_GET_PEERS, &cmd_para, NULL);
}
...
static wmg_status_t wifimg_p2p_disconnect(uint8_t *p2p_mac_addr)
{
	return wifimg_mode_ctl_cmd(P2P_MODE_NUM , WMG_P2P_CMD_DISCONNECT, p2p_mac_addr, NULL);
}
...
static wmg_status_t wifimg_register_msg_cb(wifi_msg_cb_t msg_cb ,void *msg_cb_arg)
{
	...
	if(wifimg_object.wmg_msg_cb != msg_cb) {
		WMG_DEBUG("wifimg register new msg cb\n");
		wifimg_object.wmg_msg_cb = msg_cb;
		wifimg_object.wmg_msg_cb_arg = msg_cb_arg;
...
#ifdef SUPPORT_EXPAND
static wmg_status_t wifimg_send_expand_cmd(char *expand_cmd, void *expand_params, void *expand_cb)
{
	return wmg_send_expand_cmd(expand_cmd, expand_params, expand_cb);
}
#endif
...
static wifimg_object_t wifimg_object = {
	...
	.wmg_msg_cb = NULL,
	.module_name = "UNKNOWN",
	.wmg_mode_support_list = support_list,
	.module_mode_support_list = NULL,
	...
	.p2p_get_peers = wifimg_p2p_get_peers,
