...
{
	wmg_status_t cb_msg = WMG_STATUS_FAIL;
	if(act_transfer(&wmg_act_handle,WMG_ACT_TABLE_PLA_ID,WMG_PLA_ACT_WIFI_OFF,1,1,&mode,&cb_msg)){
		WMG_ERROR("act fail\n");
		return WMG_STATUS_FAIL;
	}
	return cb_msg;
...
{
	wmg_status_t cb_msg = WMG_STATUS_FAIL;
	if(act_transfer(&wmg_act_handle,WMG_ACT_TABLE_MONITOR_ID,WMG_MONITOR_ACT_ENABLE,1,1,monitor_config,&cb_msg)){
		WMG_ERROR("act fail\n");
		return WMG_STATUS_FAIL;
	}
	return cb_msg;
...
{
	wmg_status_t cb_msg = WMG_STATUS_FAIL;
	if(act_transfer(&wmg_act_handle,WMG_ACT_TABLE_P2P_ID,WMG_P2P_ACT_FIND,2,1,&find_second,types,&cb_msg)){
		WMG_ERROR("act fail\n");
		return WMG_STATUS_FAIL;
	}
	return cb_msg;
...
{
	wmg_status_t cb_msg = WMG_STATUS_FAIL;
	if(act_transfer(&wmg_act_handle,WMG_ACT_TABLE_P2P_ID,WMG_P2P_ACT_GET_PEERS,3,1,p2p_peers,p2p_num,&p2p_arr_size,&cb_msg)){
		WMG_ERROR("act fail\n");
		return WMG_STATUS_FAIL;
	}
	return cb_msg;
...
{
	wmg_status_t cb_msg = WMG_STATUS_FAIL;
	if(act_transfer(&wmg_act_handle,WMG_ACT_TABLE_PLA_ID,WMG_PLA_ACT_SEND_EXPAND_CMD,3,1,expand_cmd,expand_params,expand_cb,&cb_msg)){
		WMG_ERROR("act fail\n");
		return WMG_STATUS_FAIL;
	}
	return cb_msg;
